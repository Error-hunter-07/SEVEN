...
def update_scratchpad_summary():
    ...
    scratchpad.scratchpad.add_tool_output(f"Updated scratchpad summary")

def update_scratchpad_seven_notes(notes):
    scratchpad.scratchpad.seven_notes = notes
    scratchpad.scratchpad.add_tool_output(f"Updated seven notes section of scratchpad")

def update_scratchpad_current_goal(goal):
    scratchpad.scratchpad.set_current_goal(goal)
    scratchpad.scratchpad.add_tool_output(f"Updated current goal to: {goal}")

def add_scratchpad_subtask(subtask):
    scratchpad.scratchpad.add_subtask(subtask)
    scratchpad.scratchpad.add_tool_output(f"Added subtask to scratchpad: {subtask}")

def add_scratchpad_retrieved_context(context):
    scratchpad.scratchpad.add_retrieved_context(context)
    scratchpad.scratchpad.add_tool_output(f"Added retrieved context to scratchpad")
...
def add_scratchpad_memory_update(update):
    ...
    scratchpad.scratchpad.add_tool_output(f"Scratchpad memory updated")
