...
def _parse_response(raw, bundle):
    ...
    return [op for op in (_parse_operation(r) for r in raw_ops) if op is not None]

def propose_operations(resolved, subgraph, bundles):
    ...
    user_content = build_operation_proposal_user(resolved_for_prompt, subgraph.text, mem_ids_texts)
    try:
        response = llm_request_lock.post_completion(
            {"model": settings.llm_model, "messages": [{"role":"system","content":_SYSTEM_PROMPT},{"role":"user","content":user_content}],
             "temperature": PIPELINE_TEMPERATURE, "max_tokens": OPERATION_PROPOSAL_MAX_TOKENS, "chat_template_kwargs": {"enable_thinking": False}},
            role="background", timeout=OPERATION_PROPOSAL_TIMEOUT)
    ...
    return _parse_response(raw, bundle)